...
for ticker in all_comms["symbol"].values:
    ...
    try:
        ...
        ticker_data["prior_high"] = ticker_data["close"].rolling(window=60).max()
        ticker_data["slow_avg"] = ticker_data["close"].rolling(window=20).mean()
        
        ticker_data["divergence"] = round(((ticker_data["close"] - ticker_data["slow_avg"]) / ticker_data["slow_avg"]) * 100, 2)
        ticker_data["t-1"] = ticker_data["divergence"].shift(1)
        ticker_data["t-2"] = ticker_data["divergence"].shift(2)
        ticker_data["t-3"] = ticker_data["divergence"].shift(3)
        ...
        ticker_data["distance_t-2"] = ticker_data["distance_from_high"].shift(2)
        ticker_data["distance_t-3"] = ticker_data["distance_from_high"].shift(3)
        ...
        plt.plot(pd.to_datetime(ticker_data["date"][-252:]), ticker_data["slow_avg"][-252:])
